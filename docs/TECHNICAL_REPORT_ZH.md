...
| 前端 | 原生 HTML/CSS/JavaScript | 无前端框架 | 三栏 Agent 工作台、运行轨迹、模型切换和对话管理 |
...
| 验证 | pytest、Node syntax check、真实浏览器验收 | 52 passed / 1 skipped | 回归 API、安全边界、桌面资源与交互行为 |
...
- 窗口改为透明外层加圆角应用边框；最大化时取消圆角和外边距。
- 标题栏固定为不可滚动区域，工作区名称居中显示。
- 三栏中央轨道改为 `minmax(0, 1fr)`，不再因最小宽度把侧栏和标题栏挤出窗口。
- 对历史保存的左右栏宽度重新执行 210–390 px、260–480 px 限幅，修复旧缓存造成的超宽溢出。
- 会话列表独立占用剩余高度并滚动，顶部导航和底部模型卡不再被挤走。

### 代码审查入口

- 点击“代码审查”只准备审查草稿和提示词，不再自动请求模型。
- 服务端提供原子 `get_or_create_empty` 逻辑，连续点击复用同一个空草稿。
- 前端增加提交锁，API 返回前再次发送不会产生重复运行。
- 运行记录绑定发起时的对话 ID；用户切换到其他对话后，任务完成不会把界面强制跳回错误页面。

## 5. 验收证据

- pytest：`52 passed, 1 skipped, 11 subtests passed`。
- JavaScript：`node --check src/doppel_agent/web/app.js` 通过。
- Python：`compileall` 通过。
- HTML 标签栈检查通过。
- 隔离工作区中连续点击“代码审查” 5 次：对话数 1、空草稿数 1、消息数 0、运行数 0。
- Edge 实际渲染检查通过：三色按钮、圆角边框、三栏布局、标题栏和输入区均可见，无左上溢出。
- PyInstaller v0.8.1 onedir EXE 构建完成。
