...
const state={settings:null,conversations:[],groups:[],current:null,runId:null,runConversationId:null,poll:null,archived:false,group:null,events:[],started:0,nextMode:'agent',submitting:false,transition:false,searchResults:[],searchIndex:0,searchTimer:null};
...
function renderConversationList(){const rows=state.conversations.filter(c=>!state.group||c.group_id===state.group);$('conversation-section-title').textContent=state.archived?'已归档':'最近对话';$('conversation-list').innerHTML=rows.length?rows.map(c=>`<button class="conversation-item ${state.current?.id===c.id?'active':''}" data-id="${c.id}"><b>${esc(c.title)}</b><span>${esc(c.group_name||formatTime(c.updated_at))} · ${c.message_count} 条消息</span></button>`).join(''):'<p class="list-empty">这里还没有对话。</p>'}
...
async function prepareNewDraft(){
  ...
  state.transition=true;setRunning(false);
  try{
    const c=await post('/api/conversations/new-draft',{});
    state.archived=false;state.group=null;state.current=c;state.nextMode='agent';
    localStorage.setItem('doppel-conversation',c.id);
    $('show-active').classList.add('active');$('show-archive').classList.remove('active');
    await refreshConversations();renderCurrent();$('prompt').value='';$('prompt').focus();
  }catch(e){alert(e.message)}finally{state.transition=false;setRunning(Boolean(state.runId))}
}
async function prepareReviewDraft(){
  ...
  try{
    ...
    $('effort-select').value='quick';$('prompt').value='';
  }catch(e){alert(e.message)}finally{state.transition=false;setRunning(Boolean(state.runId))}
}
...
function renderCurrent(){const c=state.current,empty=!c?.messages?.length,reviewDraft=empty&&c?.title==='代码审查';$('conversation-title').textContent=c?.title||'新对话';$('manage-conversation').disabled=!c;$('welcome').hidden=!empty;$('starter-options').hidden=reviewDraft;$('review-options').hidden=!reviewDraft;$('welcome-title').textContent=reviewDraft?'选择一种审查方式。':'从代码开始，而不是从猜测开始。';$('welcome-copy').textContent=reviewDraft?'只准备提示词，不会自动调用模型；确认内容后再发送。':'选择工作方式。Doppel 会绘制项目结构、定位相关行，再执行审查或修改。';$('prompt').placeholder=reviewDraft?'选择上方审查方式，或直接描述审查要求':'给 Doppel 一个任务；Enter 发送，Shift + Enter 换行';$('message-list').innerHTML=(c?.messages||[]).map(m=>`<article class="message ${m.role}"><div class="message-label">${m.role==='user'?'你':`Doppel${m.model?` · ${esc(m.model)}`:''}`}</div><div class="message-body">${m.role==='assistant'?markdown(m.content):esc(m.content).replace(/\n/g,'<br>')}</div></article>`).join('');requestAnimationFrame(()=>$('message-scroll').scrollTop=$('message-scroll').scrollHeight)}
...
function setRunning(on){
  ...
  $('run-button').disabled=busy;$('start-review').disabled=busy||state.transition;$('new-conversation').disabled=busy||state.transition;
...
function renderSearchResults(){
  ...
  $('search-results').innerHTML=rows.length?rows.map((c,i)=>`<button class="search-result ${i===state.searchIndex?'selected':''}" data-search-id="${c.id}" data-archived="${c.archived?1:0}"><b>${esc(c.title)}</b><small>${c.archived?'已归档':esc(c.group_name||formatTime(c.updated_at))}</small><span>${esc(c.preview||'空对话')}</span></button>`).join(''):'<p class="search-empty">没有找到匹配的对话。</p>';
  $('search-results').querySelector('.selected')?.scrollIntoView({block:'nearest'});
}
async function performSearch(){
  const query=$('global-search-input').value.trim();
  if(!query){state.searchResults=[];state.searchIndex=0;$('search-results').innerHTML='<p class="search-empty">输入关键词搜索所有对话，包括归档内容。</p>';return}
  try{state.searchResults=await api(`/api/conversations/search?q=${encodeURIComponent(query)}`);state.searchIndex=0;renderSearchResults()}catch(e){$('search-results').innerHTML=`<p class="search-empty">搜索失败：${esc(e.message)}</p>`}
}
function openSearch(){
  const dialog=$('search-dialog');if(!dialog.open)dialog.showModal();
  $('global-search-input').value='';state.searchResults=[];state.searchIndex=0;
  $('search-results').innerHTML='<p class="search-empty">输入关键词搜索所有对话，包括归档内容。</p>';
  requestAnimationFrame(()=>$('global-search-input').focus());
}
async function openSearchResult(index=state.searchIndex){
  const row=state.searchResults[index];if(!row)return;
  $('search-dialog').close();state.archived=Boolean(row.archived);state.group=null;
  $('show-active').classList.toggle('active',!state.archived);$('show-archive').classList.toggle('active',state.archived);
  await refreshConversations();await openConversation(row.id);
}
...
$('new-conversation').onclick=prepareNewDraft;
...
$('group-list').onclick=e=>{const b=e.target.closest('[data-group]');if(!b)return;state.group=b.dataset.group||null;renderGroups();renderConversationList()};
...
$('toggle-inspector').onclick=()=>{const hidden=document.querySelector('.app-shell').classList.toggle('inspector-hidden');$('toggle-inspector').setAttribute('aria-pressed',String(!hidden));$('toggle-inspector').title=hidden?'显示执行详情':'隐藏执行详情'};
$('open-search').onclick=openSearch;
$('global-search-input').oninput=()=>{clearTimeout(state.searchTimer);state.searchTimer=setTimeout(performSearch,140)};
$('global-search-input').onkeydown=e=>{if(e.key==='ArrowDown'&&state.searchResults.length){e.preventDefault();state.searchIndex=(state.searchIndex+1)%state.searchResults.length;renderSearchResults()}else if(e.key==='ArrowUp'&&state.searchResults.length){e.preventDefault();state.searchIndex=(state.searchIndex-1+state.searchResults.length)%state.searchResults.length;renderSearchResults()}else if(e.key==='Enter'){e.preventDefault();openSearchResult()}};
$('search-results').onclick=e=>{const b=e.target.closest('[data-search-id]');if(!b)return;const index=state.searchResults.findIndex(row=>row.id===b.dataset.searchId);openSearchResult(index)};
