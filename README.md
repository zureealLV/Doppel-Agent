...
> 当前版本：`v0.8.1`。项目仍在开发中；已实现的功能与后续计划分开列出，不以参考项目的指标作为本项目成绩。

![Doppel Agent v0.8.1 桌面工作台](docs/images/doppel-agent-v081.png)
...
- **Agent 工作台**：Ghostty 风格三色窗口按钮与圆角无边框窗口，内部采用 Codex 式三栏布局；左右栏可拖动调整，执行面板可收起，模型选择位于输入区。
- **持久对话**：消息自动写入 SQLite，支持搜索、重命名、分组、归档、删除和重启恢复；同一对话历史会继续参与模型推理。
...
左上三色按钮依次为关闭、最小化和最大化/还原，双击标题栏也可切换最大化。“代码审查”入口只会准备一个空白审查草稿并预填提示词，**不会自动消耗模型额度**；确认任务和模型后，点击输入框右下角发送按钮才会开始运行。反复点击入口会复用同一个空草稿。
